...
bandwith = int(input('Какова скорость вашего соединения? '))
chunks = math.ceil(file_size / bandwith)
print()

for chunk in range(1, chunks + 1):
  progress_mb = bandwith * chunk
  if progress_mb > file_size:
    progress_mb = file_size
  progress = math.ceil((progress_mb / file_size) * 100)
  print(f'Прошло {chunk} сек. Скачано {progress_mb} из {file_size}',
        f'Мб ({progress}%)')

print(f'\nНа скачивание файла было потрачено {chunks} сек.')
